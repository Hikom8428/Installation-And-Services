...
// Assign one or more Doers to an installation (additive — doesn't remove
// anyone already assigned), with an optional shared cash advance ("fund").
export async function PATCH(
  ...
  try {
    ...
    const updated = await prisma.installation.findUnique({ where: { id } });
...
// Unassign a single Doer: DELETE /api/installations/[id]?doerId=xxx
export async function DELETE(
  ...
  try {
    ...
    await unassignDoer("INSTALLATION", id, doerId);
    return NextResponse.json({ message: "Doer unassigned" }, { status: 200 });
  } catch (error) {
    console.error("Error unassigning installation doer:", error);
    return NextResponse.json({ message: "Internal server error" }, { status: 500 });
  }
}
